...
// WithResizePermit accounts for PTY resizes that can make tmux recompute every
// attached window. It is called only after deduplication and rate coalescing.
func WithResizePermit(permit func() error) Option {
	return func(wt *WebTTY) error {
		wt.resizePermit = permit
		return nil
	}
}
