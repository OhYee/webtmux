...
func (wt *WebTTY) handleMasterReadEvent(data []byte) error {
	...
	case ResizeTerminal:
		...
		if columns < 2 || columns > 1000 || rows < 2 || rows > 500 {
			return errors.Errorf("terminal resize outside safe bounds: %dx%d", columns, rows)
		}
		if columns == wt.lastResizeColumns && rows == wt.lastResizeRows {
			break
		}
		now := time.Now()
		if !wt.lastResizeAt.IsZero() && now.Sub(wt.lastResizeAt) < 100*time.Millisecond {
			break
		}
		if wt.resizePermit != nil {
			if err := wt.resizePermit(); err != nil {
				return err
			}
		}
		...
		wt.lastResizeColumns, wt.lastResizeRows, wt.lastResizeAt = columns, rows, now
